class Annotator:
    ...
    def annotate(self, query, first=False):
        ...
        self.stpCnt += 1
        result += "Step {}: ".format(self.stpCnt)
        ...
        if query["Node Type"] == 'Seq Scan':
            ...
            if "Filter" in query:
                msg += " filtering with {}".format(query["Filter"])
            msg += ". \n"
            ...
            table = query["Schema"]
            name = query["alias"]
            msg = "Perform function {} on schema {} and return the results as {}".format(query["Function Name"], table,
                                                                                         name)
            if "Filter" in query:
                msg += " filtering with {}".format(query["Filter"])
            msg += ". \n"
            ...
            if not first:
                ...
                msg += ". \n"
            return "T" + str(self.cnt), result + msg
            ...
            if "Merge Cond" in query:
                msg += " with the condition of {}".format(query["Merge Cond"])
            if "Filter" in query:
                msg += " filter with {}".format(query["Filter"])
            msg += ". \n"
            if not first:
                msg += " to get intermediate table T{}. \n".format(self.cnt)
            else:
            ...
            self.cnt += 1
            msg = "Perform aggregation on table {}".format(joinTables[0])
            if not first:
                msg += " to get intermediate table T{}. \n".format(self.cnt)
            else:
                msg += ". \n"
            ...
            self.cnt += 1
            msg = "Perform hash-aggregate on table {}".format(joinTables[0])
            if not first:
                msg += " to get intermediate table T{}. \n".format(self.cnt)
            else:
                msg += ". \n"
            ...
            msg = "An incremental sort is performed on table {} with sort key {}. \n".format(joinTables[0],
                                                                                            query["Sort Key"])
            return joinTables[0], result + msg
            ...
            self.cnt += 1
            msg = "Set operation is performed on table {}".format(joinTables[0])
            if not first:
                msg += " to get intermediate table T{}. \n".format(self.cnt)
            else:
                msg += ". \n"
